...
class SoftArgmax(nn.Module):
    def __init__(self, w, h, tsw, tsh, tew, teh, alpha):
        super().__init__()
        self.w = w
        self.h = h
        self.tsw = tsw # target start w
        self.tsh = tsh # target start h
        self.tew = tew # target end w
        self.teh = teh # target end h
        self.alpha = alpha

        indices_c, indices_r = torch.meshgrid(
            torch.linspace(self.tsw, self.tew, w),
            torch.linspace(self.tsh, self.teh, h),
            indexing='xy'
        )
        ...
        self.register_buffer('indices_r', indices_r, persistent=False)
        self.register_buffer('indices_c', indices_c, persistent=False)
    
    def forward(self, x):
        b, c, h, w = x.shape

        x = x.reshape(b, c, h * w)
        x = nn.functional.softmax(self.alpha * x, dim=-1)
        ...
        if h!=self.h or w!=self.w:
            ...
            indices_r = indices_r.reshape(1, h*w).to(x.device)
            indices_c = indices_c.reshape(1, h*w).to(x.device)
        else:
            indices_r = self.indices_r
            indices_c = self.indices_c


        result_r = torch.sum(x * indices_r, dim=-1) # b, c
        result_c = torch.sum(x * indices_c, dim=-1) # b, c

        result = torch.stack([result_r, result_c], dim=-1) # b, c, 2
...
class HighResolutionNet(nn.Module):
    def __init__(self, cfg, num_classes=1000, **kwargs):
        ...
        # self.softargmax = SoftArgmax(112, 112, 448, 448, 100)
        self.softargmax = SoftArgmax(64, 64, -25, -25, 281, 281, 100)
    ...
    def pred(self, x, bbox, margin=0, return_heatmap=False):
        from dataset import SynthFaceDataset_RoI

        bbox = torch.round(bbox).to(dtype=torch.int32)

        anchor = torch.clamp(bbox[:2]-margin, min=0)
        x = x[:, max(0, bbox[1]-margin):bbox[3]+margin, max(0, bbox[0]-margin):bbox[2]+margin]
        img, scale, pad = SynthFaceDataset_RoI.resize_img(x)

        with torch.no_grad():
            if return_heatmap:
                y, htmap = self.forward(img[None], return_heatmap=True)
            else:
                y = self.forward(img[None])
